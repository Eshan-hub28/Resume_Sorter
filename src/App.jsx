              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.25rem' }}>
                {manualEntries.map((entry, index) => (
                  <div key={entry.id} style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', background: 'var(--color-surface)', padding: '1rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
                    ...
                    <input type="text" placeholder="Candidate Name..." className="input-base" style={{ flex: 1, minWidth: 0, padding: '0.65rem 0.75rem', fontSize: '0.85rem', border: '1px solid var(--color-border)', fontWeight: 500 }} value={entry.name} onChange={(e) => setManualEntries(prev => prev.map(p => p.id === entry.id ? { ...p, name: e.target.value } : p))} />
                    <label className="input-base" style={{ flex: 1, minWidth: 0, padding: '0.65rem 0.75rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', cursor: 'pointer', fontSize: '0.85rem', color: entry.file ? 'var(--color-success)' : 'var(--color-text)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', fontWeight: 600, transition: 'all 0.2s', margin: 0 }} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--color-primary)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--color-border)'}>
